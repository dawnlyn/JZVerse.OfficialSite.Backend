...
using JZVerse.Business.Server.Extensions;
using JZVerse.DataAccess.PostgreSql;
...
builder.Services.AddMessageQueueClient(options =>
{
    ...
    options.ClientId = config["ClientId"] ?? "business-server";
    options.ConsumerGroup = config["ConsumerGroup"] ?? "business-group";
...
// 10. 配置热重载管理
builder.Services.AddSingleton<ConfigurationHotReloadManager>();
builder.Services.AddSingleton<ServiceRestartCoordinator>();
builder.Services.AddHostedService<ConfigurationHotReloadManager>();
