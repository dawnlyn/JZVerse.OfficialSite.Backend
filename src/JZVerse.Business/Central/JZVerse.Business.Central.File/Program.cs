...
builder.Services.AddMessageQueueClient(options =>
{
    ...
    options.ClientId = config["ClientId"] ?? "file-server";
    options.ConsumerGroup = config["ConsumerGroup"] ?? "file-group";
...
// 数据访问
var connectionString = builder.Configuration.GetConnectionString("File");
